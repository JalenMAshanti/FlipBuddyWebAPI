...
{
    public class InsertProductSpecificValueTests : BaseDataRequestTest
    {
        #region Happy Path
        [Fact]
        public async Task InsertProductSpecificValue_Given_ProductSpecificExists_ShouldReturnOneRowAffected()
        {
            //Insert User
            ...
            //Insert SpecificValue(TESTING)
            var rowsAffected = await _dataAccess.ExecuteAsync(new InsertProductSpecificValue(productSpecific_DTO.SpecificId, TestValues.SpecificValue));
            ...
            //Clean up
            await _dataAccess.ExecuteAsync(new DeleteProductSpecificValue(productSpecificValues_DTO.ValueId, productSpecificValues_DTO.SpecificId));
        ...
        #region Bad Paths
        [Fact]
        public async Task InsertProductSpecificValue_Given_ProductSpecificDoesNotExist_ShouldReturnDataAccesException()
        {
            //Insert User
            var user_DTO = await TestUser.InsertAndFetchUsersDtoAsync();
            ...
            //Insert SpecificValue(TESTING)
            var exception = await Record.ExceptionAsync(async () => await _dataAccess.ExecuteAsync(new InsertProductSpecificValue(int.MinValue, TestValues.SpecificValue)));
